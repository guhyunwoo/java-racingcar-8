...
public class OutputView {
    public void showResponse(RacingResponse racingResponse) {
        ...
        racingResponse.racingRoundInfoResponse().forEach(racingRoundInfo -> {
    ...
    private void showRoundResult(RacingRoundInfo racingRoundInfo) {
        racingRoundInfo.racingCarInfos().forEach(this::showCarProgress);
    }

    private void showCarProgress(RacingCarInfo racingCarInfo) {
        System.out.printf(OutputMessage.RACING_PROGRESS_INFO_MESSAGE.content, racingCarInfo.carName(), racingCarInfo.position());
    }
}
